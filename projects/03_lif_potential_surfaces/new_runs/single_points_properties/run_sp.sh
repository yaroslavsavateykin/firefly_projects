...
python3 "$SCRIPT_DIR/generate_sp_inputs.py"
mapfile -t INPUTS < <(find "$INPUT_DIR" -maxdepth 1 -name 'lif_sp_R_*.inp' | sort)
...
echo "Preflight for LiF single-point property runs"
echo "  Input files found=${#INPUTS[@]}"
...
[[ ${#INPUTS[@]} -gt 0 ]] || { echo "ERROR: no inputs found in $INPUT_DIR" >&2; exit 1; }
...
for inp in "${INPUTS[@]}"; do
  ...
  case "$out" in
    "$ROOT/new_runs/single_points_properties/outputs/"*) ;;
    *) echo "ERROR: output is outside single_points_properties/outputs: $out" >&2; exit 1 ;;
  esac
  echo "  cd $INPUT_DIR && P4_GLOBMEMSIZE=$P4_GLOBMEMSIZE_BYTES \"$FIREFLY_BIN\" -r -f -i \"$(basename "$inp")\" -o \"$out\" -p -stdext -ex \"$FIREFLY_EX\" -b \"$BASIS_FILE\" -t \"/tmp/firefly_${USER}_${name}_\\$\\$\" -p4pg \"/tmp/firefly_${USER}_${name}_pg_\\$\\$\" > \"$log\" 2>&1"
done
echo ""
